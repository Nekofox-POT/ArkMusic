    ...
    album_artist_list_imageObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                if (albumArtistName) {
                    const songs = ark.get_album_artist_songs(albumArtistName)
...
function album_artist_create_song_element(path, index) {
    ...
    mainArea.addEventListener('click', () => { ark.set_play_list(album_artist_batchState.list, index) })
...
async function get_album_artist_list(name) {
    ...
    // 空状态：重置
    show_frame_empty('album_artist_frame', false)
    ...
    const data = ark.get_album_artist_songs(name)

    // 空状态：检查
    if (data.length === 0) {
        show_frame_empty('album_artist_frame', true)
        set_background_color()
        return
    }
